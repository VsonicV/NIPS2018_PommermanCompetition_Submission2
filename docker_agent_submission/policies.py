...
class PommermanPolicy(Policy):
    ...
    def initialize_from(self, filename, ob_stat=None):
        ...
        with h5py.File(filename, 'r') as f:
            ...
            print('all_variables size: {}'.format(len(self.all_variables)))
            print('f_var_names size: {}'.format(len(f_var_names)))
            print(set(v.name for v in self.all_variables))
            print(set(f_var_names))
