...
class Pomme(gym.Env):
    ...
    def reset(self):
        ...
        if self._init_game_state is not None:
            ...
            for agent_id, agent in enumerate(self._agents):
                ...
                agent.reset()
                #print('[env] life_length reset to: {}'.format(agent.life_length))
                #print('[env] num_bomb_placed to: {}'.format(agent.num_bomb_placed))
    ...
    def step(self, actions):
        ...
        if done:
            ...
            for agent in self._agents:
                ...
                self._total_num_bomb_placed += agent.num_bomb_placed
    ...
    def get_agent_life_length(self):
        agent_life_lengths = []
        for agent in self._agents:
            agent_life_lengths.append(agent.life_length)
        return agent_life_lengths

    def get_num_bomb_placed(self):
        return self._total_num_bomb_placed

    def get_list_num_bomb_placed(self):
        list_num_bomb_placed = []
        for agent in self._agents:
            list_num_bomb_placed.append(agent.num_bomb_placed)
        return list_num_bomb_placed #self._total_num_bomb_placed

    def is_agent_alive(self, agent_id):
        return self._agents[agent_id].is_alive

    def get_list_agent_scores(self):
        list_agent_scores = []
        for agent in self._agents:
            list_agent_scores.append(agent.score)
        return list_agent_scores
