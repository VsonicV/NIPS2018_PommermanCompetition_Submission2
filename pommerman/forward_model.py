...
class ForwardModel(object):
                ...
                for _, indices in bomb.explode().items():
                    for r, c in indices:
                        ...
                        if curr_board[r][c] == constants.Item.Wood.value:
                            bomb.bomber.destroy_wood()
                            break
                        for agent in alive_agents:
                            r2, c2 =agent.position
                            if r==r2 and c==c2:
                                if agent.agent_id != (bomb.bomber.agent_id+2)%4 and agent.agent_id != bomb.bomber.agent_id:
                                    killed_enemy_count += 1
                                else:
                                    killed_enemy_count -= 1
                if killed_enemy_count > 0:
                    for i in range(killed_enemy_count):
                        bomb.bomber.killed_enemy()
    ...
    def get_rewards(agents, game_type, step_count, max_steps):
        ...
        if game_type == constants.GameType.FFA:
            ...
            '''
            if len(alive_agents) == 0 or step_count >= max_steps: #any_lst_equal(alive_agents, [[0, 2], [0], [2], [1, 3], [1], [3]])
                print('total number of bombs placed: {}'.format(agents[0].num_bomb_placed + agents[2].num_bomb_placed + agents[1].num_bomb_placed + agents[3].num_bomb_placed))
                print('life_length for agent 0: {}, agent 1: {}, agent 2: {}, agent 3: {}'.format(agents[0].life_length, agents[1].life_length, agents[2].life_length, agents[3].life_length))
                print('score for agent 0: {}, agent 1: {}, agent 2: {}, agent 3: {}'.format(agents[0].score, agents[1].score, agents[2].score, agents[3].score))
                if agents[0].score + agents[2].score > agents[1].score + agents[3].score:
                    return [1, -1, 1, -1]
                elif agents[0].score + agents[2].score < agents[1].score + agents[3].score:
                    return [-1, 1, -1, 1]
                elif max(agents[0].life_length, agents[2].life_length) > max(agents[1].life_length, agents[3].life_length): #any_lst_equal(alive_agents, [[0, 2], [0], [2]]):
                    # Team [0, 2] wins.
                    return [1, -1, 1, -1]
                elif max(agents[0].life_length, agents[2].life_length) < max(agents[1].life_length, agents[3].life_length): #any_lst_equal(alive_agents, [[1, 3], [1], [3]]):
                    # Team [1, 3] wins.
                    return [-1, 1, -1, 1]
                else:
                    return [-1] * 4
